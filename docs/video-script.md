...
Saadhak is an autonomous options trading agent on Alpaca. It finds its own trades, sizes them, places them, and closes them, with no one watching. What makes it different is that it decides how much to risk based on how accurate it has actually been. Most A.I. trading agents are confident. This one is calibrated.

## 0:22 — What it trades

Here is what it trades. Every position is a defined-risk options structure, usually an iron condor, which profits when the market stays inside a range. It sells that range one day before expiry, on index products liquid enough to get out of. It never sells anything naked, and it never sends a market order. The most it can lose on any position is known before the order goes in.
